...
//         if(Yval[i]<=0 || Yval[i]-Yerr[i]){Yval[i]=1E-5; Yerr[i]=0;}
         if(i==0)sprintf(YAxisTitle,"Acceptance (%%) for M#kern[-0.5]{_{req.}}= %i GeV/c#kern[-0.8]{^{2}}",(int)dmsVector[i][DrawIndex].massCut);
         break;
      }
   }
   ...
   graph->GetXaxis()->SetTitle(PlotVsWidth ? "log_{10} [ #chi_{1}^{#pm} c#tau (m) ]" : "HSCP Mass (GeV/c#kern[-0.8]{^{2}})");
...
void MakePlot_PMSSM(){
      ...
      TH1D* HCtau  = new TH1D("HCtau" , "HCtau;log_{10} [ #chi_{1}^{#pm} c#tau (m) ];# points", 200, -19,18 );
      TH1D* HCtauS = new TH1D("HCtauS", "HCtau;log_{10} [ #chi_{1}^{#pm} c#tau (m) ];# points", 200, -19,18 );
      TH1D* HCtauE = new TH1D("HCtauE", "HCtau;log_{10} [ #chi_{1}^{#pm} c#tau (m) ];# points", 200, -19,18 );
      TH1D* HCtauL = new TH1D("HCtauL", "HCtau;log_{10} [ #chi_{1}^{#pm} c#tau (m) ];# points", 200, -19,18 );
      TH1D* HCtauM = new TH1D("HCtauM", "HCtau;log_{10} [ #chi_{1}^{#pm} c#tau (m) ];# points", 200, -19,18 );
...
//        char buffer[255]; sprintf(buffer, "pictures/pMSSMInc_%i.0.txt", (int)Titr);
        ...
        if(Tctau<0.01 && pointSigma[(int)Titr]<-1.64){  //ECLUDED BY SUSY
           ...
           printf("POINT THAT FAILLED : M=%f decaylength=%Em  THXsec=%E  LimitXsec=%E\n", Tmass, Tctau, results[MassCut].ThXSec, results[MassCut].Limits[5]);
...
//           }
      ...
      printf("Number of point excluded = %f)\n", HCtauS->Integral());
      printf("NTEST=%i NTEST0=%i NTEST1=%i NTEST2=%i\n", NTEST, NTEST0, NTEST1, NTEST2);


     printf("Faction of non excluded model with ctau>10m = %f\n", HCtauE->Integral(HCtauE->FindBin(1), 10000) / HCtau->Integral());
     printf("Faction of non excluded model with ctau>1m = %f\n", HCtauE->Integral(HCtauE->FindBin(0), 10000) / HCtau->Integral());
     printf("Faction of non excluded model with ctau>50cm = %f\n", HCtauE->Integral(HCtauE->FindBin(log10(0.5)), 10000) / HCtau->Integral());
     printf("Faction of non excluded model with ctau<10mm = %f\n", HCtauS->Integral(-1, HCtauS->FindBin(log10(0.01))) / HCtau->Integral());
   ...
   //     T->AddText("CMS Preliminary  -  #sqrt{s} = 8 TeV  -  Simulation");
        ...
        TPad* t1 = new TPad("t1","t1", 0.0, 0.25, 1.0, 0.95);
        t1->Draw();
        t1->cd();
        t1->SetTopMargin(0.05);
        t1->SetBottomMargin(0.17);
        t1->SetLogy(true);
...
//        stack->Add(HCtau);
   ...
   //     t2->SetLogy(true);
        ...
        TH1D* frameR =  new TH1D("frameR", "frameR", 1,PLOT==0?stack->GetXaxis()->GetXmin():-1.0, PLOT==0?stack->GetXaxis()->GetXmax():6.0);
        frameR->SetTitle("");
        frameR->SetStats(kFALSE);
        frameR->GetXaxis()->SetTitleOffset(1.45);
        frameR->GetYaxis()->SetTitle("Excluded Fraction");
        frameR->GetYaxis()->SetTitleOffset(1.45);
        frameR->SetMaximum(1.05);   
        frameR->SetMinimum(0);
        frameR->GetXaxis()->SetLabelFont(43); //give the font size in pixel (instead of fraction)
        frameR->GetXaxis()->SetLabelSize(18); //font size
        frameR->GetXaxis()->SetTitleFont(43); //give the font size in pixel (instead of fraction)
        frameR->GetXaxis()->SetTitleSize(18); //font size
        frameR->GetXaxis()->SetNdivisions(510);
        frameR->GetYaxis()->SetLabelFont(43); //give the font size in pixel (instead of fraction)
        frameR->GetYaxis()->SetLabelSize(18); //font size
        ...
        TH1D* HCtauSRatio = ((TH1D*) HCtauS->Clone("HCtauSRatio")); HCtauSRatio->Divide(HCtau);  if(PLOT==0)HCtauSRatio->Draw("same");
         ...
         c1->cd(0);
   /*
         TPaveText* T = new TPaveText(0.0, 0.97, 1.0, 0.99, "NDC");
         T->SetTextFont(43);  //give the font size in pixel (instead of fraction)
         T->SetTextSize(21);  //font size
         T->SetFillColor(0);
         T->SetTextAlign(22);
...
//         stack->GetXaxis()->SetRangeUser(-1.0,6.0);
      ...
      T = new TPaveText(0.25,0.93,0.5,0.85, "NDC");   T->SetFillColor(0); T->SetTextAlign(12); T->SetTextFont(43); T->SetTextSize(21); T->AddText("Raw pMSSM LHC subspace");  T->Draw("same");
      ...
      c1 = new TCanvas("c1", "c1", 600, 600);
      c1->SetLogz(true);
      HMML->GetYaxis()->SetTitleOffset(1.80);
      HMML->Draw("CONTZ");
      HMML->GetXaxis()->SetLabelFont(43); //give the font size in pixel (instead of fraction)
      HMML->GetXaxis()->SetLabelSize(18); //font size
      HMML->GetXaxis()->SetTitleFont(43); //give the font size in pixel (instead of fraction)
      HMML->GetXaxis()->SetTitleSize(18); //font size
      HMML->GetXaxis()->SetNdivisions(510);
      HMML->GetYaxis()->SetLabelFont(43); //give the font size in pixel (instead of fraction)
      HMML->GetYaxis()->SetLabelSize(18); //font size
      HMML->GetYaxis()->SetTitleFont(43); //give the font size in pixel (instead of fraction)
      HMML->GetYaxis()->SetTitleSize(18); //font size
      HMML->GetYaxis()->SetNdivisions(510);
      T = new TPaveText(0.0, 0.97, 1.0, 0.99, "NDC"); T->SetFillColor(0); T->SetTextAlign(22); T->SetTextFont(43); T->SetTextSize(21); T->AddText("CMS Preliminary  -  #sqrt{s} = 8 TeV  -   L = 18.8 fb^{-1}");      T->Draw("same");
      T = new TPaveText(0.25,0.93,0.5,0.85, "NDC");   T->SetFillColor(0); T->SetTextAlign(12); T->SetTextFont(43); T->SetTextSize(21); T->AddText("Not excluded by EXO-13-006"); T->AddText("nor SUS-12-030");  T->Draw("same");
...
//   return;
   ...
   for(unsigned int M=0;M<5+7;M++){
      ...
      switch(M){
         ...
         case 5:
            ModelName = "pMSSM_M100";;             LegendName= "pMSSM #chi_{1}^{#pm} (M = 100GeV/c#kern[-0.8]{^{2}})";
            dms_ST = dms_pMSSMTeresa_ST[0];
            dms_MI = dms_pMSSMTeresa_MI[0];
            break;

         case 6:
            ModelName = "pMSSM_M200";;             LegendName="pMSSM #chi_{1}^{#pm} (M = 200GeV/c#kern[-0.8]{^{2}})";
            dms_ST = dms_pMSSMTeresa_ST[1];
            dms_MI = dms_pMSSMTeresa_MI[1];
            break;

         case 7:
            ModelName = "pMSSM_M300";;             LegendName="pMSSM #chi_{1}^{#pm} (M = 300GeV/c#kern[-0.8]{^{2}})";
            dms_ST = dms_pMSSMTeresa_ST[2];
            dms_MI = dms_pMSSMTeresa_MI[2];
            break;

         case 8:
            ModelName = "pMSSM_M400";;             LegendName="pMSSM #chi_{1}^{#pm} (M = 400GeV/c#kern[-0.8]{^{2}})";
            dms_ST = dms_pMSSMTeresa_ST[3];
            dms_MI = dms_pMSSMTeresa_MI[3];
            break;

         case 9:
            ModelName = "pMSSM_M500";;             LegendName="pMSSM #chi_{1}^{#pm} (M = 500GeV/c#kern[-0.8]{^{2}})";
            dms_ST = dms_pMSSMTeresa_ST[4];
            dms_MI = dms_pMSSMTeresa_MI[4];
            break;

         case 10:
            ModelName = "pMSSM_M600";;             LegendName="pMSSM #chi_{1}^{#pm} (M = 600GeV/c#kern[-0.8]{^{2}})";
            dms_ST = dms_pMSSMTeresa_ST[5];
            dms_MI = dms_pMSSMTeresa_MI[5];
            break;

         case 11:
            ModelName = "pMSSM_M700";;             LegendName="pMSSM #chi_{1}^{#pm} (M = 700GeV/c#kern[-0.8]{^{2}})";
...
//      std::vector< std::vector<dataModel> >& dms_MI = (M==0)?dms_PP_MI:(M==1)?dms_GM_MI:(M==2)?dms_DY_MI:(M==3)?dms_pMSSM_MI:dms_pMSSMMO_MI;
      ...
      for(int LevelIndex=-2; LevelIndex<6;LevelIndex++){
         ...
         TH1D* frame = NULL;
         if(M<=4){
            frame = new TH1D("frame", "frame; Mass (GeV/c#kern[-0.8]{^{2}})", 1,0,M<2?1100:1100);
            frame->GetXaxis()->SetTitle("Mass (GeV/c#kern[-0.8]{^{2}})");
         }else{
      ...
      //   TLegend* leg = new TLegend(0.5,0.13,0.88,0.30);
         TLegend* leg = new TLegend(0.45,0.20,0.88,0.50);
         leg->SetFillStyle(0);
         leg->SetBorderSize(0);
         leg->SetTextFont(43);
         leg->SetTextSize(20);
         ...
         if(LevelIndex>0 && M<=4){
            ...
            b->SetFillStyle(3004);
            b->Draw("same");
            leg->AddEntry(b, "M < 1.6 M#kern[-0.5]{_{req.}}" ,"F");
         }else if(M>4){
         ...
         frameR->GetYaxis()->SetTitleOffset(1.45);
         frameR->SetMaximum(1.25);   
         frameR->SetMinimum(0.75);
         frameR->GetXaxis()->SetLabelFont(43); //give the font size in pixel (instead of fraction)
         frameR->GetXaxis()->SetLabelSize(18); //font size
         frameR->GetXaxis()->SetTitleFont(43); //give the font size in pixel (instead of fraction)
         frameR->GetXaxis()->SetTitleSize(18); //font size
...
//         MI_limit1S->Draw("f same");
      ...
      //   TLegend* leg = new TLegend(0.5,0.13,0.88,0.30);
         TLegend* leg = new TLegend(0.45,0.92,0.88,0.66);
         leg->SetFillStyle(0);
         leg->SetBorderSize(0);
         leg->SetTextFont(43);
         leg->SetTextSize(20);
...
//         leg->AddEntry(MI_limit1S, "Exp. #pm1 #sigma unc." ,"F");
         ...
         t2->SetGridx(true);
         t2->SetGridy(true);
         t2->SetPad(0,0.0,1.0,0.25);
         t2->SetTopMargin(0.05);
         t2->SetBottomMargin(0.17);
...
/////////////////////////////////////////////////  PLOT OF PROC EFFICIENCY
   ...
   for(unsigned int M=0;M<2;M++){
      ...
      switch(M){
         case 0:
            ModelName = "pMSSMInc2";             LegendName="#chi_{1}^{#pm} (Stable, M = 200GeV/c#kern[-0.8]{^{2}})";
         ...
         case 1:
            ModelName = "pMSSMInc7";             LegendName="#chi_{1}^{#pm} (Stable, M = 700GeV/c#kern[-0.8]{^{2}})";
         ...
         TH1D* frame = NULL;
         frame = new TH1D("frame", "frame; Mass (GeV/c#kern[-0.8]{^{2}})", 4,Xmin, Xmax);
      ...
      //   TLegend* leg = new TLegend(0.5,0.13,0.88,0.30);
         TLegend* leg = NULL;
         if(M==0){leg=new TLegend(0.45,0.70,0.88,0.90);}else{leg = new TLegend(0.45,0.20,0.88,0.50);}
         leg->SetFillStyle(0);
         leg->SetBorderSize(0);
         leg->SetTextFont(43);
         leg->SetTextSize(20);
