...
bool isIsolated(const std::vector<reco::GenParticle>& genColl, const reco::GenParticle& cand){
      double caloIso=0.0;
      double tkIso=0.0;
      for(unsigned int g=0;g<genColl.size();g++){
//         if(genColl[g].status()!=2)continue;
         ...
         printf("pdg = %i status=%i  charge=%i\n", genColl[g].pdgId(), genColl[g].status(), genColl[g].charge());
