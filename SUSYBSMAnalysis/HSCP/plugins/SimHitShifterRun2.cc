...
{
  ShiftFileName  = iConfig.getUntrackedParameter<std::string>("ShiftFileName","/afs/cern.ch/user/c/carrillo/simhits/CMSSW_3_5_8_patch2/src/simhitshifter/SimHitShifterRun2/Merged_Muon_RawId_Shift.txt");
...
void SimHitShifterRun2::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
   ...
   for (std::vector<PSimHit>::const_iterator iHit = theSimHits.begin(); iHit != theSimHits.end(); iHit++){
	 ...
	 newtof = (*iHit).timeOfFlight()+shiftinfo[simdetid.rawId()];
       ...
       PSimHit hit((*iHit).entryPoint(),(*iHit).exitPoint(),(*iHit).pabs(),  newtof,  (*iHit).energyLoss(),(*iHit).particleType(),simdetid,(*iHit). trackId(),(*iHit).thetaAtEntry(),(*iHit).phiAtEntry(),(*iHit).processType());
       prpc->push_back(hit);
     }
	 ...
	 std::cout<<"DT Warning the RawId = "<<RawId<<"leading to condensed Id: " << (RawId&0xFFC3FFFF) << " is not in the map"<<std::endl;
	 newtof = (*iHit).timeOfFlight();
       }else{
	 ...
	 newtof = (*iHit).timeOfFlight()+shiftinfo[RawId&0xFFC3FFFF];
       ...
       if(shiftinfo.find(RawId&0xFFFFFE07)==shiftinfo.end()){
         std::cout<<"CSC Warning the RawId = "<<RawId<<"leading to condensed Id: " << (RawId&0xFFFFFE07) << " is not in the map"<<std::endl;
         newtof = (*iHit).timeOfFlight();
       }else{
...
void 
SimHitShifterRun2::beginRun(const edm::Run& run, const edm::EventSetup& iSetup)
{
...
void 
SimHitShifterRun2::beginJob(const edm::Run& run, const edm::EventSetup& iSetup)
{
