...
*/
   ...
   if(MODsColl.size()==0){
      ...
      vector<const GeomDet*> Det = tkGeom->dets();
      for(unsigned int i=0;i<Det.size();i++){
         ...
         if( SubDet == StripSubdetector::TIB ||  SubDet == StripSubdetector::TID ||
             ...
             const StripGeomDetUnit* DetUnit     = dynamic_cast<const StripGeomDetUnit*> (Det[i]);
             if(!DetUnit)continue;
            ...
            const PixelGeomDetUnit* DetUnit = dynamic_cast<const PixelGeomDetUnit*> (Det[i]);
             if(!DetUnit)continue;
